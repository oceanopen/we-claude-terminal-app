// 全量重扫：discover → enrich → 写 ClaudeSessionStore → emit claude-sessions:changed。
//
...
use crate::shared::state::claude_sessions::{ClaudeSessionStore, write_claude_sessions};
use crate::shared::types::{ClaudeSessionInfo, TerminalApp};
...
pub fn rescan(app: &AppHandle) {
    ...
    let sessions: Vec<ClaudeSessionInfo> = raws
        .iter()
        .map(enrich::enrich)
